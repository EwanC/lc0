...
void OpenCL::initialize(const int channels, const OpenCLParams& params) {
  ...
  for (const auto& p : platforms) {
    ...
    for (auto& d : devices) {
      ...
      // deliberately overweight command-buffer extension availability to aid
      // graph development
      this_score += (d.getInfo<CL_DEVICE_EXTENSIONS>().find(
  ...
  m_graph_capture_enabled = params.graph_capture;
